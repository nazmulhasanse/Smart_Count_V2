...
$sql = "SELECT 
    ...
    $sql = "SELECT 
            ...
            FROM
                eworker_operation o
                    LEFT JOIN
                eworker_assignment w ON o.docnumber = w.docnumber
                    AND o.Style = w.Style
...
<?php
    ...
    while ($row = $queryResult->fetch_assoc()) {
      ...
      $data .= "{ label: $Time, y: $HourlyProductionQty},";
    ...
    while ($row = $queryResult->fetch_assoc()) {
      ...
      if($HourlyProductionQty == '') $HourlyProductionQty = '0';
        ...
        WHERE
            o.DateTime LIKE '%$Date%'
                AND o.qty_type IN ('pass', 'rework-pass', 'reject', 'rework-reject')
                AND w.Position = '2'
                AND o.docnumber = '$docnumber'
                AND SUBSTRING(o.DateTime, 12, 2) = '$Timeq' LIMIT 1";
        ...
        $defectQuantity = $HourlyProductionQty - $FinishedGoodsQty;

        $data2 .=  "{ label: $Time, y: $defectQuantity},";
    ...
    while ($row = $queryResult->fetch_assoc()) {
      ...
      if($HourlyProductionQty == '') $HourlyProductionQty = '0';
        ...
        $defectPercentage = number_format((($HourlyProductionQty - $FinishedGoodsQty) / $HourlyProductionQty) * 100, 2);

        $data3 .=  "{ label: $Time, y: $defectPercentage},";
