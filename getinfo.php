...
<?php
    ...
    $sql = "SELECT
            ...
            WHERE
                ...
                AND o.size != '' 
                AND o.SewingLine LIKE '$Division%'
                AND SUBSTRING(o.`DateTime`, 1, 10) = '$Date'
            GROUP BY
                o.color,
                o.size
            ORDER BY
    ...
    $sql = "SELECT SUBSTRING(`o`.`DateTime`, 1, 7) AS `Month`, SUM(`o`.`Qty`) AS `MonthlyPassProductionQty` FROM `eworker_operation` `o` LEFT JOIN `ework_workers` w ON `o`.`cardnumber` = `w`.`cardnumber` WHERE `o`.`qty_type` IN ('pass', 'rework-pass') AND `w`.`Position` = '2' AND SUBSTRING(`o`.`DateTime`, 1, 7) = SUBSTRING('$Date', 1, 7) AND `o`.`SewingLine` LIKE '$Division%'";
    ...
    $sql = "SELECT SUBSTRING(`o`.`DateTime`, 1, 7) AS `Month`, SUM(`o`.`Qty`) AS `MonthlyFailProductionQty` FROM `eworker_operation` `o` LEFT JOIN `ework_workers` w ON `o`.`cardnumber` = `w`.`cardnumber` WHERE `o`.`qty_type` IN ('reject', 'rework-reject') AND `w`.`Position` = '2' AND SUBSTRING(`o`.`DateTime`, 1, 7) = SUBSTRING('$Date', 1, 7) AND `o`.`SewingLine` LIKE '$Division%'";
