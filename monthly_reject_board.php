...
<?php
    ...
    $sql2 = "SELECT SUBSTRING(`o`.`DateTime`, 1, 10) AS `Day`, SUM(`o`.`Qty`) AS `DailyProductionFailQty` FROM `eworker_operation` `o` LEFT JOIN `ework_workers` w ON `o`.`cardnumber` = `w`.`cardnumber` WHERE `o`.`qty_type` IN ('reject', 'rework-reject') AND `w`.`Position` = '2' AND SUBSTRING(`o`.`DateTime`, 1, 10) LIKE '$Date%' AND `o`.`SewingLine` = '$Line' GROUP BY SUBSTRING(`o`.`DateTime`, 1, 10) ORDER BY SUBSTRING(`o`.`DateTime`, 1, 10) ASC";
