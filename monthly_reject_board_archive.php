...
<?php
    ...
    $sql = "SELECT SUBSTRING(`o`.`DateTime`, 1, 10) AS `Day`, SUM(`o`.`Qty`) AS `DailyProductionQty` FROM `eworker_operation_archive` `o` LEFT JOIN `ework_workers` w ON `o`.`cardnumber` = `w`.`cardnumber` WHERE `o`.`qty_type` IN ('pass', 'fail', 'reject') AND `w`.`Position` = '2' AND SUBSTRING(`o`.`DateTime`, 1, 10) LIKE '$Date%' AND `o`.`SewingLine` = '$Line' GROUP BY SUBSTRING(`o`.`DateTime`, 1, 10) ORDER BY SUBSTRING(`o`.`DateTime`, 1, 10) ASC";
    ...
    $sql2 = "SELECT SUBSTRING(`o`.`DateTime`, 1, 10) AS `Day`, SUM(`o`.`Qty`) AS `DailyProductionFailQty` FROM `eworker_operation_archive` `o` LEFT JOIN `ework_workers` w ON `o`.`cardnumber` = `w`.`cardnumber` WHERE `o`.`qty_type` IN ('reject', 'rework-reject') AND `w`.`Position` = '2' AND SUBSTRING(`o`.`DateTime`, 1, 10) LIKE '$Date%' AND `o`.`SewingLine` = '$Line' GROUP BY SUBSTRING(`o`.`DateTime`, 1, 10) ORDER BY SUBSTRING(`o`.`DateTime`, 1, 10) ASC";
    ...
    while ($row2 = $queryResult2->fetch_assoc()) {
      ...
      $data2 .= "{ label: '$Day', y: $DailyProductionFailQty, color: 'black'},";
        ...
        $data3 .=  "{ label: '$Day', y: $defectPercentage, color: 'orange'},";
...
window.onload = function () {
    ...
    {   
        ...
        theme: "light2",
        title: {
            text: "Monthly Reject Qty & Reject Rate (%)",
            fontSize: 30
        },
    ...
    // },
    {
        ...
        indexLabel: "{y}",
        indexLabelOrientation: "vertical", // Add this line
        indexLabelFontColor: "red", // Add this line
        // indexLabelPlacement: "inside",
        // showInLegend: true,
